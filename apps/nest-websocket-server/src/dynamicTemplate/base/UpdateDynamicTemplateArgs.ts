/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DynamicTemplateWhereUniqueInput } from "./DynamicTemplateWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { DynamicTemplateUpdateInput } from "./DynamicTemplateUpdateInput";

@ArgsType()
class UpdateDynamicTemplateArgs {
  @ApiProperty({
    required: true,
    type: () => DynamicTemplateWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => DynamicTemplateWhereUniqueInput)
  @Field(() => DynamicTemplateWhereUniqueInput, { nullable: false })
  where!: DynamicTemplateWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => DynamicTemplateUpdateInput,
  })
  @ValidateNested()
  @Type(() => DynamicTemplateUpdateInput)
  @Field(() => DynamicTemplateUpdateInput, { nullable: false })
  data!: DynamicTemplateUpdateInput;
}

export { UpdateDynamicTemplateArgs as UpdateDynamicTemplateArgs };
